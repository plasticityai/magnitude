...
class HigherOrderType(ComplexType):
    ...
    second : ``Type``
        Passed to NLTK's ComplexType.
    """
    def __init__(self, num_arguments     , first      , second      )        :
        super(HigherOrderType, self).__init__(first, second)
...
class UnaryOpType(PlaceholderType):
    ...
    def __init__(self,
                 ...
                 signature      = u'<#1,#1>')        :
        super(UnaryOpType, self).__init__(type_, type_)
...
class BinaryOpType(PlaceholderType):
    ...
    def __init__(self,
                 ...
                 allowed_substitutions                 = None,
                 signature      = u'<#1,<#1,#1>>')        :
        super(BinaryOpType, self).__init__(type_, ComplexType(type_, type_))
        self._allowed_substitutions = allowed_substitutions
        self._signature = signature
...
class DynamicTypeApplicationExpression(ApplicationExpression):
    ...
    """
    def __init__(self, function            , argument            , variables_with_placeholders          )        :
    ...
    def type(self):
        ...
        # to return the type.
        if not unicode(self.function) in self._variables_with_placeholders:
    ...
    def _set_type(self, other_type       = ANY_TYPE, signature=None)        :
        ...
        if isinstance(self.argument, ApplicationExpression) and unicode(self.argument.function) == u"V":
            # pylint: disable=protected-access
            self.argument.argument._set_type(self.function.type.first)
        if unicode(self.argument) == u"X" and unicode(self.function) != u"V":
            # pylint: disable=protected-access
...
class DynamicTypeLogicParser(LogicParser):
    ...
    def __init__(self,
        ...
        self._variables_with_placeholders = set([name for name, type_ in list(type_signatures.items())
    ...
    def make_VariableExpression(self, name):
        if u":" in name:
            ...
            if prefix in self._constant_type_prefixes:
                ...
                raise RuntimeError("Unknown prefix: {prefix}. Did you forget to pass it to the constructor?")
...
def _make_production_string(source      , target                         )       :
...
def get_valid_actions(name_mapping                ,
                      type_signatures                 ,
                      basic_types           ,
                      valid_starting_types            = None,
    ...
    for type_ in valid_starting_types:
        valid_actions[unicode(START_TYPE)].add(_make_production_string(START_TYPE, type_))

    complex_types = set()
    for name, alias in list(name_mapping.items()):
        ...
        if name in [u"lambda", u"var", u"x", u"y", u"z"]:
            continue
        ...
        for substituted_type in substitute_any_type(name_type, basic_types):
            valid_actions[unicode(substituted_type)].add(_make_production_string(substituted_type, name))
    ...
    for complex_type in complex_types:
        for substituted_type in substitute_any_type(complex_type, basic_types):
            ...
            valid_actions[unicode(head)].add(production)
    ...
    for i in range(num_nested_lambdas):
        ...
        for first_type in basic_types:
            for second_type in basic_types:
                ...
                production_string = _make_production_string(key, [u'lambda ' + lambda_var, second_type])
                valid_actions[unicode(key)].add(production_string)
...
LAMBDA_VARIABLES = set([u'x', u'y', u'z'])
