...
class TestEndpointSpanExtractor(object):
    def test_endpoint_span_extractor_can_build_from_params(self):
        params = Params({
                u"type": u"endpoint",
                u"input_dim": 7,
                u"num_width_embeddings": 5,
                u"span_width_embedding_dim": 3
                })
    ...
    def test_masked_indices_are_handled_correctly_with_exclusive_indices(self):
        ...
        extractor = EndpointSpanExtractor(8, u"x,y", use_exclusive_start_indices=True)
        indices = torch.LongTensor([[[1, 3],
                                     [2, 4]],
                                    [[0, 2],
                                     [0, 1]]])
        ...
        correct_end_embeddings = batched_index_select(sequence_tensor.contiguous(),
                                                      correct_end_indices)
        numpy.testing.assert_array_equal(end_embeddings.data.numpy(),
                                         correct_end_embeddings.data.numpy())
