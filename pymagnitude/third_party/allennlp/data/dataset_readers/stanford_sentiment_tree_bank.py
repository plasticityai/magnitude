...
class StanfordSentimentTreeBankDatasetReader(DatasetReader):
    ...
    def __init__(self,
                 token_indexers                          = None,
                 use_subtrees       = False,
                 granularity      = u"5-class",
                 lazy       = False)        :
        super(StanfordSentimentTreeBankDatasetReader, self).__init__(lazy=lazy)
        self._token_indexers = token_indexers or {u'tokens': SingleIdTokenIndexer()}
        self._use_subtrees = use_subtrees
        allowed_granularities = [u"5-class", u"3-class", u"2-class"]
        if granularity not in allowed_granularities:
            raise ConfigurationError(u"granularity is {}, but expected one of: {}".format(
...
StanfordSentimentTreeBankDatasetReader = DatasetReader.register(u"sst_tokens")(StanfordSentimentTreeBankDatasetReader)
