...
class WikiTablesDatasetReader(DatasetReader):
    ...
    def _read(self, file_path     ):
        if file_path.endswith(u'.examples'):
            yield self._read_examples_file(file_path)
        elif file_path.endswith(u'.jsonl'):
            yield self._read_preprocessed_file(file_path)
        else:
            raise ConfigurationError("Don't know how to read filetype of {file_path}")

    def _read_examples_file(self, file_path     ):
        ...
        if self._dpd_output_directory:
            logger.info("Missing DPD info for {num_dpd_missing} out of {num_lines} instances")
            num_with_dpd = num_lines - num_dpd_missing
            num_bad_lfs = num_with_dpd - num_instances
            logger.info("DPD output was bad for {num_bad_lfs} out of {num_with_dpd} instances")
            if num_bad_lfs > 0:
                logger.info(u"Re-run with log level set to debug to see the un-parseable logical forms")
            logger.info("Kept {num_instances} instances")

    def _read_preprocessed_file(self, file_path     ):
        with open(file_path, u"r") as data_file:
            for line in data_file.readlines():
    ...
    def text_to_instance(self,  # type: ignore
        ...
        metadata                 = {u"question_tokens": [x.text for x in tokenized_question]}
        metadata[u"original_table"] = u"".join(table_lines)
        ...
        for production_rule in world.all_possible_actions():
            _, rule_right_side = production_rule.split(u' -> ')
            is_global_rule = not world.is_table_entity(rule_right_side)
            field = ProductionRuleField(production_rule, is_global_rule)
            production_rule_fields.append(field)
        action_field = ListField(production_rule_fields)

        fields = {u'question': question_field,
                  u'metadata': MetadataField(metadata),
                  u'table': table_field,
                  u'world': world_field,
                  u'actions': action_field}
        if self._include_table_metadata:
            fields[u'table_metadata'] = table_metadata
        if example_lisp_string:
            fields[u'example_lisp_string'] = MetadataField(example_lisp_string)

        # We'll make each target action sequence a List[IndexField], where the index is into
        # the action list we made above.  We need to ignore the type here because mypy doesn't
        # like `action.rule` - it's hard to tell mypy that the ListField is made up of
        # ProductionRuleFields.
        action_map = dict((action.rule, i) for i, action in enumerate(action_field.field_list))  # type: ignore
        if dpd_output:
            action_sequence_fields              = []
            for logical_form in dpd_output:
                if not self._should_keep_logical_form(logical_form):
                    logger.debug('Question was: {question}')
                    logger.debug('Table info was: {table_lines}')
                    continue
                try:
                    expression = world.parse_logical_form(logical_form)
                except ParsingError as error:
                    logger.debug('Parsing error: {error.message}, skipping logical form')
                    logger.debug('Question was: {question}')
                    logger.debug('Logical form was: {logical_form}')
                    logger.debug('Table info was: {table_lines}')
                    continue
                except:
                ...
                try:
                    ...
                    logger.debug('Missing production rule: {error.args}, skipping logical form')
                    logger.debug('Question was: {question}')
                    logger.debug('Table info was: {table_lines}')
                    logger.debug('Logical form was: {logical_form}')
                    continue
        ...
        if self._output_agendas:
            ...
            fields[u'agenda'] = ListField(agenda_index_fields)
        return Instance(fields)

    def _json_blob_to_instance(self, json_obj          )            :
        question_tokens = self._read_tokens_from_json_list(json_obj[u'question_tokens'])
        question_field = TextField(question_tokens, self._question_token_indexers)
        question_metadata = MetadataField({u"question_tokens": [x.text for x in question_tokens]})
        table_knowledge_graph = TableQuestionKnowledgeGraph.read_from_lines(json_obj[u'table_lines'],
                                                                            question_tokens)
        entity_tokens = [self._read_tokens_from_json_list(token_list)
                         for token_list in json_obj[u'entity_texts']]
        table_field = KnowledgeGraphField(table_knowledge_graph,
                                          question_tokens,
                                          tokenizer=None,
                                          token_indexers=self._table_token_indexers,
                                          entity_tokens=entity_tokens,
                                          linking_features=json_obj[u'linking_features'],
                                          include_in_vocab=self._use_table_for_vocab,
                                          max_table_tokens=self._max_table_tokens)
        world = WikiTablesWorld(table_knowledge_graph)
        world_field = MetadataField(world)
        ...
        example_string_field = MetadataField(json_obj[u'example_lisp_string'])

        fields = {u'question': question_field,
                  u'metadata': question_metadata,
                  u'table': table_field,
                  u'world': world_field,
                  u'actions': action_field,
                  u'example_lisp_string': example_string_field}

        if u'target_action_sequences' in json_obj or u'agenda' in json_obj:
            action_map = dict((action.rule, i) for i, action in enumerate(action_field.field_list))  # type: ignore
        if u'target_action_sequences' in json_obj:
            action_sequence_fields              = []
            for sequence in json_obj[u'target_action_sequences']:
                index_fields              = []
                for production_rule in sequence:
                    index_fields.append(IndexField(action_map[production_rule], action_field))
                action_sequence_fields.append(ListField(index_fields))
            fields[u'target_action_sequences'] = ListField(action_sequence_fields)
        if u'agenda' in json_obj:
            agenda_index_fields              = []
            for agenda_action in json_obj[u'agenda']:
                agenda_index_fields.append(IndexField(action_map[agenda_action], action_field))
            fields[u'agenda'] = ListField(agenda_index_fields)
        return Instance(fields)

    @staticmethod
    def _read_tokens_from_json_list(json_list)               :
        return [Token(text=json_obj[u'text'], lemma=json_obj[u'lemma']) for json_obj in json_list]
    ...
    def _parse_example_line(lisp_string     )                  :
        ...
        question, rest = rest.split(u'") (context (graph tables.TableKnowledgeGraph ')
        table_filename, rest = rest.split(u')) (targetValue (list')
        return {u'id': example_id, u'question': question, u'table_filename': table_filename}
    ...
    def _should_keep_logical_form(logical_form     )        :
        ...
        if logical_form.count(u'(or') > 3:
            logger.debug('Skipping logical form with inordinate number of "ors": {logical_form}')
            return False
        return True

WikiTablesDatasetReader = DatasetReader.register(u"wikitables")(WikiTablesDatasetReader)
