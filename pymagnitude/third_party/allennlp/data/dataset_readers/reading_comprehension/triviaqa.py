...
class TriviaQaReader(DatasetReader):
    ...
    def __init__(self,
                 base_tarball_path     ,
                 unfiltered_tarball_path      = None,
                 tokenizer            = None,
                 token_indexers                          = None,
                 lazy       = False)        :
        super(TriviaQaReader, self).__init__(lazy)
    ...
    def _read(self, file_path     ):
        logger.info(u"Opening base tarball file at %s", self._base_tarball_path)
        base_tarball = tarfile.open(cached_path(self._base_tarball_path), u'r')
        if u'unfiltered' in file_path:
            logger.info(u"Opening unfiltered tarball file at %s", self._unfiltered_tarball_path)
            unfiltered_tarball = tarfile.open(cached_path(self._unfiltered_tarball_path), u'r')
            logger.info(u"Loading question file from tarball")
            data_json = json.loads(unfiltered_tarball.extractfile(file_path).read().decode(u'utf-8'))
        else:
            logger.info(u"Loading question file from tarball")
            path = os.path.join(u'qa', file_path)
            data_json = json.loads(base_tarball.extractfile(path).read().decode(u'utf-8'))

        logger.info(u"Reading the dataset")
        for question_json in data_json[u'Data']:
            question_text = question_json[u'Question']
            question_tokens = self._tokenizer.tokenize(question_text)

            evidence_files                  = []  # contains lines from each evidence file
            if u'web' in file_path:
                for result in question_json[u'SearchResults']:
                    filename = result[u'Filename']
                    evidence_file = base_tarball.extractfile(os.path.join(u"evidence", u"web", filename))
                    evidence_files.append([line.decode(u'utf-8') for line in evidence_file.readlines()])
            else:
                for result in question_json[u'EntityPages']:
                    filename = result[u'Filename']
                    evidence_file = base_tarball.extractfile(os.path.join(u"evidence", u"wikipedia", filename))
                    evidence_files.append([line.decode(u'utf-8') for line in evidence_file.readlines()])

            answer_json = question_json[u'Answer']
            human_answers = [util.normalize_text(answer) for answer in answer_json.get(u'HumanAnswers', [])]
            answer_texts = answer_json[u'NormalizedAliases'] + human_answers
