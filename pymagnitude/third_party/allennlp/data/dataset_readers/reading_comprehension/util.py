...
# scripts.
IGNORED_TOKENS = set([u'a', u'an', u'the'])
STRIPPED_CHARACTERS = string.punctuation + u''.join([u"‘", u"’", u"´", u"`", u"_"])
...
def char_span_to_token_span(token_offsets                       ,
    ...
    if token_offsets[start_index][0] > character_span[0]:
        ...
        logger.debug(u"Bad labelling or tokenization - start offset doesn't match")
        start_index -= 1
    if token_offsets[start_index][0] != character_span[0]:
        error = True
    ...
    if end_index == start_index and token_offsets[end_index][1] > character_span[1]:
        ...
        # token.
        logger.debug(u"Bad tokenization - end offset doesn't match")
        ...
        logger.debug(u"Bad labelling or tokenization - end offset doesn't match")
...
def make_reading_comprehension_instance(question_tokens             ,
    ...
    fields[u'passage'] = passage_field
    fields[u'question'] = TextField(question_tokens, token_indexers)
    metadata = {
            u'original_passage': passage_text,
            u'token_offsets': passage_offsets,
            u'question_tokens': [token.text for token in question_tokens],
            u'passage_tokens': [token.text for token in passage_tokens],
            }
    if answer_texts:
        metadata[u'answer_texts'] = answer_texts
    ...
    if token_spans:
        ...
        fields[u'span_start'] = IndexField(span_start, passage_field)
        fields[u'span_end'] = IndexField(span_end, passage_field)
