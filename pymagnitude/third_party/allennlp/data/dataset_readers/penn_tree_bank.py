...
class PennTreeBankConstituencySpanDatasetReader(DatasetReader):
    ...
    def __init__(self,
                 ...
                 use_pos_tags       = True,
                 lazy       = False)        :
        super(PennTreeBankConstituencySpanDatasetReader, self).__init__(lazy=lazy)
    ...
    def text_to_instance(self, # type: ignore
        ...
        if self._use_pos_tags and pos_tags is not None:
            pos_tag_field = SequenceLabelField(pos_tags, text_field, label_namespace=u"pos")
            fields[u"pos_tags"] = pos_tag_field
        elif self._use_pos_tags:
            raise ConfigurationError(u"use_pos_tags was set to True but no gold pos"
        ...
        if gold_tree is not None:
            fields[u"span_labels"] = SequenceLabelField(gold_labels,
                                                       span_list_field)
    ...
    def _strip_functional_tags(self, tree      )        :
        ...
        """
        clean_label = tree.label().split(u"=")[0].split(u"-")[0].split(u"|")[0]
        tree.set_label(clean_label)
        for child in tree:
            if not isinstance(child[0], unicode):
...
PennTreeBankConstituencySpanDatasetReader = DatasetReader.register(u"ptb_trees")(PennTreeBankConstituencySpanDatasetReader)
