...
class InvalidTagSequence(Exception):
    def __init__(self, tag_sequence=None):
        super(InvalidTagSequence, self).__init__()
        self.tag_sequence = tag_sequence

    def __str__(self):
        return u' '.join(self.tag_sequence)
...
def bio_tags_to_spans(tag_sequence           ,
    ...
    for index, string_tag in enumerate(tag_sequence):
        ...
        if bio_tag not in [u"B", u"I", u"O"]:
            raise InvalidTagSequence(tag_sequence)
        conll_tag = string_tag[2:]
        if bio_tag == u"O" or conll_tag in classes_to_ignore:
            # The span has ended.
            ...
            span_start = index
            span_end = index
        elif bio_tag == u"I" and conll_tag == active_conll_tag:
            # We're inside a span.
            span_end += 1
        else:
...
def iob1_tags_to_spans(tag_sequence           ,
    ...
    for index, string_tag in enumerate(tag_sequence):
        ...
        if curr_bio_tag not in [u"B", u"I", u"O"]:
            raise InvalidTagSequence(tag_sequence)
        if curr_bio_tag == u"O" or curr_conll_tag in classes_to_ignore:
            # The span has ended.
            if active_conll_tag is not None:
                spans.add((active_conll_tag, (span_start, span_end)))
            active_conll_tag = None
...
def _iob1_start_of_chunk(prev_bio_tag               ,
                         prev_conll_tag               ,
                         curr_bio_tag     ,
                         curr_conll_tag     )        :
    if curr_bio_tag == u'B':
        return True
    if curr_bio_tag == u"I" and prev_bio_tag == u"O":
        return True
    if curr_bio_tag != u'O' and prev_conll_tag != curr_conll_tag:
        return True
    return False
...
def bioul_tags_to_spans(tag_sequence           ,
    ...
    while index < len(tag_sequence):
        ...
        if label[0] == u'U':
            spans.append((label.partition(u'-')[2], (index, index)))
        elif label[0] == u'B':
            start = index
            while label[0] != u'L':
                index += 1
                if index >= len(tag_sequence):
                    raise InvalidTagSequence(tag_sequence)
                label = tag_sequence[index]
                if not (label[0] == u'I' or label[0] == u'L'):
                    raise InvalidTagSequence(tag_sequence)
            spans.append((label.partition(u'-')[2], (start, index)))
        else:
            if label != u'O':
                raise InvalidTagSequence(tag_sequence)
        index += 1
...
def iob1_to_bioul(tag_sequence           )             :
    warnings.warn(u"iob1_to_bioul has been replaced with 'to_bioul' "
                  u"to allow more encoding options.", FutureWarning)
...
def to_bioul(tag_sequence           , encoding      = u"IOB1")             :
    ...
    if not encoding in set([u"IOB1", u"BIO"]):
        raise ConfigurationError("Invalid encoding {encoding} passed to 'to_bioul'.")
    # pylint: disable=len-as-condition

    def replace_label(full_label, new_label):
        # example: full_label = 'I-PER', new_label = 'U', returns 'U-PER'
        parts = list(full_label.partition(u'-'))
        parts[0] = new_label
        return u''.join(parts)
    ...
    def process_stack(stack, out_stack):
        ...
        if len(stack) == 1:
            # just a U token
            pop_replace_append(stack, out_stack, u'U')
        else:
            # need to code as BIL
            recoded_stack = []
            pop_replace_append(stack, recoded_stack, u'L')
            while len(stack) >= 2:
                pop_replace_append(stack, recoded_stack, u'I')
            pop_replace_append(stack, recoded_stack, u'B')
    ...
    for label in tag_sequence:
        ...
        if label == u'O' and len(stack) == 0:
            bioul_sequence.append(label)
        elif label == u'O' and len(stack) > 0:
            ...
            if len(stack) == 0:
                ...
                # check if the previous type is the same as this one
                this_type = label.partition(u'-')[2]
                prev_type = stack[-1].partition(u'-')[2]
