...
TypedStringSpan = Tuple[unicode, Tuple[int, int]]  # pylint: disable=invalid-name
...
class Ontonotes(object):
    ...
    def dataset_iterator(self, file_path     )                               :
        ...
        for conll_file in self.dataset_path_iterator(file_path):
            yield self.sentence_iterator(conll_file)
    ...
    def dataset_path_iterator(file_path     )                 :
        ...
        for root, _, files in list(os.walk(file_path)):
            for data_file in files:
                ...
                if not data_file.endswith(u"gold_conll"):
                    continue
    ...
    def dataset_document_iterator(self, file_path     )                                     :
        ...
        """
        with codecs.open(file_path, u'r', encoding=u'utf8') as open_file:
            conll_rows = []
            document                          = []
            for line in open_file:
                line = line.strip()
                if line != u'' and not line.startswith(u'#'):
                    ...
                    if conll_rows:
                        ...
                        conll_rows = []
                if line.startswith(u"#end document"):
                    yield document
                    document = []
            if document:
    ...
    def _conll_rows_to_sentence(self, conll_rows           )                     :
        ...
        for index, row in enumerate(conll_rows):
            ...
            if pos_tag != u"XX" and word != u"XX":
                ...
                if pos_tag == u')':
                    pos_tag = u'-RRB-'
                (left_brackets, right_hand_side) = parse_piece.split(u'*')
                # only keep ')' if there are nested brackets with nothing in them.
                right_brackets = right_hand_side.count(u')') * u')'
                parse_piece = '{left_brackets} ({pos_tag} {parse_word}) {right_brackets}'
            else:
            ...
            word_is_verbal_predicate = any([u"(V" in x for x in conll_components[11:-1]])
            ...
            predicate_lemmas.append(lemmatised_word if lemmatised_word != u"-" else None)
            predicate_framenet_ids.append(framenet_id if framenet_id != u"-" else None)
            word_senses.append(float(word_sense) if word_sense != u"-" else None)
            speakers.append(speaker if speaker != u"-" else None)

        named_entities = span_labels[0]
        srl_frames = [(predicate, labels) for predicate, labels
                      in izip(verbal_predicates, span_labels[1:])]

        if all(parse_pieces):
            parse_tree = Tree.fromstring(u"".join(parse_pieces))
        else:
            parse_tree = None
        coref_span_tuples                 = set([(cluster_id, span)
                                             for cluster_id, span_list in list(clusters.items())
    ...
    def _process_span_annotations_for_word(annotations           ,
        ...
        for annotation_index, annotation in enumerate(annotations):
            ...
            if u"(" in annotation:
                ...
                bio_label = u"I-" + current_span_labels[annotation_index]
                span_labels[annotation_index].append(bio_label)
            else:
                # We're outside a span.
                span_labels[annotation_index].append(u"O")
