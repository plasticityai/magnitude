...
class Model(torch.nn.Module, Registrable):
        ...
        with torch.no_grad():
            ...
            for name, output in list(outputs.items()):
                if isinstance(output, torch.Tensor):
                    ...
                    continue
                outputs[name] = output
                for instance_output, batch_element in izip(instance_separated_output, output):
    ...
    def _maybe_warn_for_unseparable_batches(self, output_key     ):
        ...
        if  output_key not in self._warn_for_unseparable_batches:
            logger.warning("Encountered the {output_key} key in the model's return dictionary which "
                           u"couldn't be split by the batch size. Key will be ignored.")
    ...
    def load(cls,
        ...
        model_type = config[u"model"][u"type"]
...
def remove_pretrained_embedding_params(params        ):
    ...
    if u'pretrained_file' in keys:
        del params[u'pretrained_file']
