...
class WikiTablesErmSemanticParser(WikiTablesSemanticParser):
    ...
    def __init__(self,
        ...
        for production, index in list(global_vocab.items()):
            right_side = production.split(u" -> ")[1]
    ...
    def _initialize_weights_from_archive(self, archive         )        :
        ...
        question_embedder_weight = u"_question_embedder.token_embedder_tokens.weight"
        if question_embedder_weight not in archived_parameters or\
           question_embedder_weight not in model_parameters:
            raise RuntimeError(u"When initializing model weights from an MML model, we need "
                               u"the question embedder to be a TokenEmbedder using namespace called "
                               u"tokens.")
    ...
    def forward(self,  # type: ignore
        ...
        for instance_actions, instance_agenda, terminal_productions in izip(actions,
                                                                           agenda_list,
        ...
        initial_state = initial_info[u"initial_state"]
        ...
        if not self.training:
            ...
            linking_scores = initial_info[u"linking_scores"]
            feature_scores = initial_info[u"feature_scores"]
            similarity_scores = initial_info[u"similarity_scores"]
            batch_size = list(question.values())[0].size(0)
            action_mapping = {}
            for batch_index, batch_actions in enumerate(actions):
                for action_index, action in enumerate(batch_actions):
                    action_mapping[(batch_index, action_index)] = action[0]
            outputs[u'action_mapping'] = action_mapping
            outputs[u'entities'] = []
            outputs[u'linking_scores'] = linking_scores
            if feature_scores is not None:
                outputs[u'feature_scores'] = feature_scores
            outputs[u'similarity_scores'] = similarity_scores
            outputs[u'logical_form'] = []
            best_action_sequences = outputs[u'best_action_sequences']
            outputs[u"best_action_sequence"] = []
            outputs[u'debug_info'] = []
            ...
            for i in range(batch_size):
                ...
                if i in best_action_sequences:
                    ...
                    outputs[u'best_action_sequence'].append([])
                    outputs[u'logical_form'][-1].append(u'')
                    self._has_logical_form(0.0)
                    if example_lisp_string:
                        self._denotation_accuracy(None, example_lisp_string[i])
                self._agenda_coverage(in_agenda_ratio)
        if metadata is not None:
            outputs[u"question_tokens"] = [x[u"question_tokens"] for x in metadata]
            outputs[u"original_table"] = [x[u"original_table"] for x in metadata]
        return outputs
...
WikiTablesErmSemanticParser = Model.register(u"wikitables_erm_parser")(WikiTablesErmSemanticParser)
