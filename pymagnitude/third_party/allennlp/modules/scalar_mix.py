...
class ScalarMix(torch.nn.Module):
    ...
    def forward(self, tensors                    ,  # pylint: disable=arguments-differ
        ...
        if len(tensors) != self.mixture_size:
            raise ConfigurationError(u"{} tensors were passed, but the module was initialized to "
                                     u"mix {} tensors.".format(len(tensors), self.mixture_size))
        ...
        if not self.do_layer_norm:
            pieces = []
            for weight, tensor in izip(normed_weights, tensors):
