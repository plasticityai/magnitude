...
class BiMpmMatching(nn.Module, FromParams):
    ...
    def __init__(self,
        ...
        if not (with_full_match or with_maxpool_match or with_attentive_match or with_max_attentive_match):
            raise ConfigurationError(u"At least one of the matching method should be enabled")
        ...
        if with_full_match:
            if is_forward is None:
                raise ConfigurationError(u"Must specify is_forward to enable full matching")
