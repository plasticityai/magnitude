...
class BidirectionalEndpointSpanExtractor(SpanExtractor):
    ...
    def __init__(self,
                 input_dim     ,
                 forward_combination      = u"y-x",
                 backward_combination      = u"x-y",
                 num_width_embeddings      = None,
                 span_width_embedding_dim      = None,
                 bucket_widths       = False,
                 use_sentinels       = True)        :
        super(BidirectionalEndpointSpanExtractor, self).__init__()
        ...
        if self._input_dim % 2 != 0:
            raise ConfigurationError(u"The input dimension is not divisible by 2, but the "
                                     u"BidirectionalEndpointSpanExtractor assumes the embedded representation "
                                     u"is bidirectional (and hence divisible by 2).")
        if num_width_embeddings is not None and span_width_embedding_dim is not None:
            ...
            raise ConfigurationError(u"To use a span width embedding representation, you must"
                                     u"specify both num_width_buckets and span_width_embedding_dim.")
        else:
    ...
    def forward(self,
        ...
        if (exclusive_span_starts < 0).any() or (exclusive_span_ends > sequence_lengths.unsqueeze(-1)).any():
            raise ValueError("Adjusted span indices must lie inside the length of the sequence tensor, "
                             "but found: exclusive_span_starts: {exclusive_span_starts}, "
                             "exclusive_span_ends: {exclusive_span_ends} for a sequence tensor with lengths "
                             "{sequence_lengths}.")
...
BidirectionalEndpointSpanExtractor = SpanExtractor.register(u"bidirectional_endpoint")(BidirectionalEndpointSpanExtractor)
