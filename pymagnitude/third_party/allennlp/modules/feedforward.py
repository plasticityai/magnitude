...
class FeedForward(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if len(hidden_dims) != num_layers:
            raise ConfigurationError(u"len(hidden_dims) (%d) != num_layers (%d)" %
                                     (len(hidden_dims), num_layers))
        if len(activations) != num_layers:
            raise ConfigurationError(u"len(activations) (%d) != num_layers (%d)" %
        ...
        linear_layers = []
        for layer_input_dim, layer_output_dim in izip(input_dims, hidden_dims):
    ...
    def forward(self, inputs              )                :
        ...
        output = inputs
        for layer, activation, dropout in izip(self._linear_layers, self._activations, self._dropout):
