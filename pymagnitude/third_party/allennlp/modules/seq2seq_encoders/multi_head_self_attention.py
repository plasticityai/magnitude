...
class MultiHeadSelfAttention(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        if attention_dim % num_heads != 0:
            raise ValueError("Key size ({attention_dim}) must be divisible by the number of "
                             "attention heads ({num_heads}).")

        if values_dim % num_heads != 0:
            raise ValueError("Value size ({values_dim}) must be divisible by the number of "
                             "attention heads ({num_heads}).")
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        _3to2list = list(combined_projection.split(self._attention_dim, -1))
        # split by attention dim - if values_dim > attention_dim, we will get more
        # than 3 elements returned. All of the rest are the values vector, so we
        # just concatenate them back together again below.
        queries, keys, values, = _3to2list[:2] + [_3to2list[2:]]
...
MultiHeadSelfAttention = Seq2SeqEncoder.register(u"multi_head_self_attention")(MultiHeadSelfAttention)
