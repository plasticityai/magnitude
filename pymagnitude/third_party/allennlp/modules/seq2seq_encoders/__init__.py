...
class _Seq2SeqWrapper(object):
    ...
    def from_params(self, params        )                         :
        if not params.pop_bool(u'batch_first', True):
...
Seq2SeqEncoder.register(u"gru")(_Seq2SeqWrapper(torch.nn.GRU))
Seq2SeqEncoder.register(u"lstm")(_Seq2SeqWrapper(torch.nn.LSTM))
Seq2SeqEncoder.register(u"rnn")(_Seq2SeqWrapper(torch.nn.RNN))
Seq2SeqEncoder.register(u"augmented_lstm")(_Seq2SeqWrapper(AugmentedLstm))
Seq2SeqEncoder.register(u"alternating_lstm")(_Seq2SeqWrapper(StackedAlternatingLstm))
Seq2SeqEncoder.register(u"stacked_bidirectional_lstm")(_Seq2SeqWrapper(StackedBidirectionalLstm))
if torch.cuda.is_available():
    try:
        ...
        Seq2SeqEncoder.register(u"alternating_highway_lstm_cuda")(_Seq2SeqWrapper(AlternatingHighwayLSTM))
    except (ModuleNotFoundError, FileNotFoundError, ImportError):
        logger.debug(u"allennlp could not register 'alternating_highway_lstm_cuda' - installation "
                     u"needs to be completed manually if you have pip-installed the package. "
                     u"Run ``bash make.sh`` in the 'custom_extensions' module on a machine with a "
                     u"GPU.")
