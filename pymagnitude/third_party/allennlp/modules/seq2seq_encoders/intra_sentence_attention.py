    ...
    def __init__(self,
        ...
        if isinstance(similarity_function, MultiHeadedSimilarity):
            if num_attention_heads == 1:
                raise ConfigurationError(u"Similarity function has multiple heads but encoder doesn't")
            if num_attention_heads != similarity_function.num_heads:
                raise ConfigurationError(u"Number of heads don't match between similarity function "
                                         u"and encoder: %d, %d" % (num_attention_heads,
                                                                  similarity_function.num_heads))
        elif num_attention_heads > 1:
            raise ConfigurationError(u"Encoder has multiple heads but similarity function doesn't")
...
IntraSentenceAttentionEncoder = Seq2SeqEncoder.register(u"intra_sentence_attention")(IntraSentenceAttentionEncoder)
